...
import {
  Activity, FileText, CheckCircle, XCircle, Clock,
...
function generateRandom() {
  return {
    ...
    itemId: `ITM-${Math.random().toString(36).substr(2, 8).toUpperCase()}`,
...
export default function TransactionLive() {
  ...
  const validateForm = () => {
    ...
    if (!formData.paymentMethod) e.paymentMethod = 'Required'
    setErrors(e)
  ...
  const InputField = ({ label, field, placeholder, required }) => (
    <div>
      ...
      <input type="text" value={formData[field]} onChange={(e) => handleChange(field, e.target.value)}
        className={`w-full px-3 py-2 bg-gray-800 border rounded-lg text-white text-sm ${errors[field] ? 'border-red-500' : 'border-gray-700'} focus:border-indigo-500 focus:outline-none`}
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-1">Submit transactions and watch the agent evaluate risk in real-time</p>
        </div>
        <div className="flex items-center gap-2">
          <a href="/transactions" className="px-4 py-2 bg-gray-800 hover:bg-gray-700 text-gray-300 text-sm font-medium rounded-lg border border-gray-700 transition-colors flex items-center gap-2">
            <FileText className="w-4 h-4" /> View All Transactions
          </a>
          <button onClick={handleNewSubmission} disabled={submitting} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-semibold rounded-lg transition-colors flex items-center gap-2 disabled:opacity-50">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="bg-[#12121a] rounded-xl border border-gray-800 p-4">
              <h3 className="font-semibold text-white mb-3 flex items-center gap-2 text-sm">
                <Activity className="w-4 h-4 text-indigo-400" /> Transaction Details
              </h3>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={formData.paymentMethod} onChange={(e) => handleChange('paymentMethod', e.target.value)}
                    className={`w-full px-3 py-2 bg-gray-800 border rounded-lg text-white text-sm ${errors.paymentMethod ? 'border-red-500' : 'border-gray-700'} focus:border-indigo-500 focus:outline-none`}>
                    {paymentMethods.map(m => <option key={m} value={m}>{m.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</option>)}
                  </select>
                  {errors.paymentMethod && <p className="text-xs text-red-400 mt-1">{errors.paymentMethod}</p>}
                </div>
                <InputField label="Item ID" field="itemId" placeholder="ITM-XXXXX" />
            ...
            <button type="submit" disabled={submitting} className="w-full py-3 bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500 text-white font-semibold rounded-xl flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all">
          ...
          {result?.pending && !showDecision && (
            <div className="mt-4"><div className="bg-indigo-500/10 border border-indigo-500/30 rounded-xl p-4">
              <div className="flex items-center gap-3"><Loader className="w-5 h-5 text-indigo-400 animate-spin" /><div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">Transaction: {result.entityId}</p>
              </div></div>
            </div></div>
          )}
