...
// ---------------------------------------------------------------------------
// GET / — List policy enforcement checks with optional filtering
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// GET /stats — Aggregate statistics for policy enforcement checks
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// GET /:id — Retrieve a single policy enforcement check by ID
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// POST / — Create a new policy enforcement check and fire-and-forget agent
// ---------------------------------------------------------------------------
router.post('/', async (req, res) => {
  try {
    ...
    const checkId = `POL-${Date.now()}-${Math.random().toString(36).substr(2, 6).toUpperCase()}`;
    const correlationId = `POL-${Date.now().toString(36).toUpperCase()}-${Math.random().toString(36).slice(2, 6).toUpperCase()}`;
    ...
    // Fire-and-forget: run the policy enforcement agent
    const agent = getPolicyEnforcementAgent();
    agent.reason(
      {
      ...
      {
        entityId: checkId,
        evaluationType: 'policy_enforcement',
...
// ---------------------------------------------------------------------------
// PATCH /:id/status — Update the status of a policy enforcement check
// ---------------------------------------------------------------------------
router.patch('/:id/status', async (req, res) => {
  try {
    ...
    const validStatuses = ['CLEAR', 'WARN', 'RESTRICT', 'EVALUATING'];
