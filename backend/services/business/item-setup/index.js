...
router.get('/stats', (req, res) => {
  try {
    ...
    for (const record of all) {
      ...
      totalVariants += record.variants || 0;
    }

    res.json({
      success: true,
      data: { total: all.length, byStatus, byCategory, flagged, totalVariants }
    });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
...
router.post('/', async (req, res) => {
  try {
    const { sellerId, title, category, price, weight, complianceData } = req.body;
    ...
    const id = `ITEM-${Date.now().toString(36).toUpperCase()}-${Math.random().toString(36).substring(2, 6).toUpperCase()}`;
    const correlationId = `ITEM-${Date.now().toString(36).toUpperCase()}-${Math.random().toString(36).slice(2, 6).toUpperCase()}`;

    const record = {
      [ID_FIELD]: id, ...req.body,
      status: 'EVALUATING', riskScore: null, riskFactors: [],
    ...
    res.status(202).json({
      success: true, correlationId, entityId: id, status: 'EVALUATING',
      message: 'Agent evaluation started. Watch the Agent Flow panel for real-time progress.'
    });

    console.log(`[ItemSetupService] Evaluating: ${id} (correlation: ${correlationId})`);

    const agent = getItemSetupAgent();
    agent.reason({
      type: 'item_setup_evaluation', itemId: id, sellerId, title, category, price, weight,
      complianceData, sellerData: seller.data, submittedAt: new Date().toISOString()
    }, { entityId: id, evaluationType: 'item_setup', _correlationId: correlationId })
      .then(agentResult => {
        ...
        const agentId = agentResult.result?.agentId || 'ITEM_SETUP';
        ...
        db_ops.update(COLLECTION, record.id || id, {
        ...
        emitRiskEvent({
          sellerId, domain: 'item_setup',
          eventType: decision === 'APPROVE' ? 'ITEM_APPROVED' : decision === 'REJECT' ? 'ITEM_REJECTED' : 'ITEM_FLAGGED',
        ...
        if (decision !== 'APPROVE') {
          ...
          db_ops.insert('cases', 'case_id', caseId, {
            caseId, checkpoint: 'ITEM_SETUP', priority: riskScore >= 80 ? 'CRITICAL' : riskScore >= 60 ? 'HIGH' : 'MEDIUM',
            status: 'OPEN', sellerId, entityId: id, entityType: 'ITEM_SETUP',
        ...
        console.log(`[ItemSetupService] Completed: ${id} → ${decision} (risk: ${riskScore})`);
      })
      .catch(error => {
        console.error(`[ItemSetupService] Agent error for ${id}:`, error.message);
        db_ops.update(COLLECTION, record.id || id, {
          ...record, status: 'PENDING_REVIEW', riskScore: 50,
          riskAssessment: { riskScore: 50, decision: 'FLAG', reasoning: `Agent error — defaulting to FLAG: ${error.message}`, agentId: 'ITEM_SETUP', evaluatedAt: new Date().toISOString() },
          updatedAt: new Date().toISOString()
        });
        try {
          import('../../../gateway/websocket/event-bus.js').then(({ getEventBus }) => {
