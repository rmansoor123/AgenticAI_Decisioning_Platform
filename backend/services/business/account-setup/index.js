...
router.post('/', (req, res) => {
  try {
    ...
    if (req.body.brandInfringement) {
      riskScore += 30;
      riskFactors.push('brandInfringement');
    }
    if (req.body.sharedPaymentMethod) {
      riskScore += 40;
      riskFactors.push('sharedPaymentMethod');
    }
    if (req.body.timezoneCurrencyAnomaly) {
      riskScore += 15;
      riskFactors.push('timezoneCurrencyAnomaly');
    }
    if (!req.body.taxConfigComplete) {
      riskScore += 10;
      riskFactors.push('taxConfigIncomplete');
    }
