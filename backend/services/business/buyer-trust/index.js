...
router.get('/', async (req, res) => {
  try {
    ...
    let records = await db_ops.getAll('buyer_checks') || [];
...
router.get('/stats', async (req, res) => {
  try {
    const records = await db_ops.getAll('buyer_checks') || [];
...
router.get('/:id', async (req, res) => {
  try {
    const record = await db_ops.getById('buyer_checks', 'check_id', req.params.id);
    if (!record) {
...
router.post('/', async (req, res) => {
  try {
    ...
    await db_ops.insert('buyer_checks', 'check_id', checkId, record);
    ...
    agent.reason(
      ...
      .then(async agentResult => {
        ...
        await db_ops.update('buyer_checks', 'check_id', checkId, {
          ...record,
          status: decision,
          riskScore,
          riskAssessment: {
...
router.patch('/:id/status', async (req, res) => {
  try {
    ...
    await db_ops.update('buyer_checks', 'check_id', req.params.id, updated);
