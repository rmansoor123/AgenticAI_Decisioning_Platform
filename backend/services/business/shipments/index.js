...
const COLLECTION = 'outbound_shipments';
const ID_FIELD = 'shipment_id';
...
router.get('/stats', (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: { total: all.length, byStatus, byCarrier, flagged }
    });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
...
router.post('/', (req, res) => {
  try {
    const id = `SHIP-${Date.now().toString(36).toUpperCase()}-${Math.random().toString(36).substring(2, 6).toUpperCase()}`;
    let riskScore = 0;
    const riskFactors = [];

    if (req.body.noMatchingOrder) {
      riskScore += 50;
      riskFactors.push('noMatchingOrder');
    }
    if (req.body.weightDiscrepancy) {
      riskScore += 25;
      riskFactors.push('weightDiscrepancy');
    }
    if (req.body.carrierMismatch) {
      riskScore += 10;
      riskFactors.push('carrierMismatch');
    }
    if (req.body.dropShipSuspected) {
      riskScore += 15;
      riskFactors.push('dropShipSuspected');
    }
